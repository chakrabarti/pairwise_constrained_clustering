...
class Result():
    ...
    def __init__(self, algorithm, num_clusters, run_id=0, save_path=None):
        ...
        # The algorithm is run NUM_SETS times, each set containing 1000 uniform random samples
        self.run_id = run_id
        self.save_path = save_path
        self.running_time = 0

        # every tuple in the list is printed on a new line and every element of the tuple is printed with spaces
        self.sim_info = []
        self.sim_info.append([self.algorithm + " Simulation Parameters"])

        # List of two tuples, the first one containing cost description and the second one contains numerical cost value
        self.cost = [[], []]
    ...
    def refresh(self):
        ...
        self.cost[1][3] = round(
            ((self.cost[1][1] - self.cost[1][0]) / self.cost[1][1]) * 100, 2)
        self.cost[1][4] = round(
            ((self.cost[1][2] - self.cost[1][1]) / self.cost[1][1]) * 100, 2)
    ...
    def dump(self):
        ...
        target = os.path.join(self.save_path, str(
            self.num_clusters) + "_clusters")
        try:
            os.makedirs(target)
        except:
        ...
        target = os.path.join(target, self.algorithm +
                              "_" + self.run_id + ".result")
        file = open(target, "wb")
    ...
    def save(self, dataset, path):
        ...
        for i in range(len(self.single_axis_plot)):
            ...
            SingleAxisPlot(
                ...
                self.single_axis_plot[i][4] + " - " + dataset +
                " dataset, " + str(self.num_clusters) + " clusters"
            )

        # Generate Secondary Axis Plots
        for i in range(len(self.secondary_axis_plot)):
            target = os.path.join(
                path, self.secondary_axis_plot[i][6] + ".jpg")
            SecondaryAxisPlot(
                ...
                self.secondary_axis_plot[i][6] + " - " + dataset +
                " dataset, " + str(self.num_clusters) + " clusters"
            )
